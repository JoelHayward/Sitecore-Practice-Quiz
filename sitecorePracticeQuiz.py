...
Question("What is the function of the `layers.config` file in Sitecore?", "It defines the layers and the order in which they load at runtime", ["It stores user data", "It contains security settings", "It manages network configurations"]), Question("What is the purpose of Sitecore's Experience Platform (XP)?", "To create personalized experiences across all channels", ["To manage user permissions", "To improve website speed", "To build and deploy websites"]), Question("How does Sitecore's personalization feature work?", "It uses the Rule Set Editor to add rules and actions to a specific component, using these rules to display targeted, relevant content based on the characteristics and behavior of the contacts", ["It manually adjusts the website content for each user", "It changes the website layout based on user preferences", "It tracks user interactions across multiple channels"]), Question("What are the core roles in Sitecore?", "These are instances of the core Sitecore application with certain features enabled or disabled. In a fully scaled deployment, a dedicated instance performs each role", ["These are user roles with specific permissions", "These are the main modules of the Sitecore platform", "These are the main components of the Sitecore database"]), Question("What is the main benefit of using Sitecore as a CMS?", "It provides a comprehensive solution for managing content and customer experiences across multiple channels", ["It is the fastest CMS available", "It offers the best security features", "It supports the most programming languages"]), Question("What is a patch file in Sitecore?", "A file that is used to modify settings in the default Sitecore configuration", ["A file that contains security updates", "A file that is used to fix bugs in the system", "A file that is used to add new features to the system"]), Question("How can you control the sequence in which patch files load in Sitecore?", "By adding a <loadOrder> setting to the layer", ["By renaming the patch files", "By moving the patch files to a different folder", "By changing the file extensions"]), Question("In what order does Sitecore load files within a configuration layer?", "Sitecore loads the files in each subfolder in alphabetical order, with files in the root of a folder merging before files in subfolders within the folder", ["Sitecore loads the files in the order they were created", "Sitecore loads the files in reverse alphabetical order", "Sitecore loads the files randomly"]), Question("What is the main benefit of using xDB in Sitecore?", "It collects and connects customer data from multiple channels to provide a comprehensive view of each customer, enabling personalization and effective marketing", ["It speeds up the website load times", "It manages user permissions", "It secures customer data"]), Question("What does xConnect do in Sitecore?", "It serves as an intermediary between your application and xConnect, allowing you to read, write, and search for contacts and interactions", ["It manages the website themes", "It improves website load times", "It secures customer data"]),
    ...
    Question(
        ...
        ["/Sitecore/Home", "/Sitecore/Item/Home", "/inetpub/wwroot/Web/Home"]
    ),
    Question(
        "What's the recommended practices for a multisite implementation?",
        "All of the above",
        ["Creating a folder for each site", "Ensuring that each site folder has its own Home item", "Storing shared content outside of each site folder"]
    ),
    Question(
        "What version of Visual Studio works with the Sitecore project?",
        "Visual Studio 2015 or higher",
        ["Visual Studio 2012", "Visual Studio 2014", "Visual Studio 2015"]
    ),
    Question(
        "Visual Studio is the main tool to:",
        "All of the above",
        ["Configure and manage Sitecore project", "Code Layouts and renderings", "Manage configuration files"]
    ),
    Question(
        "Sitecore Rocks allows you to:",
        "All of the above",
        [
            "Create and manage site content from the Visual Studio",
            "Create Presentation component items and code files",
            "Integrate with your Visual Studio project",
    ...
    Question(
        "Templates define:",
        "Type of item, Determine the field sections, Determine the field names, Determine the field types",
        ["All of the above"]
    ),
    Question(
    ...
    Question(
        ...
        ["Master to Core database", "Core to Web database", "None of the above"]
    ),
    Question(
    ...
    Question(
        ...
        "Nuget",
        ["Copy and paste the library from a bin folder", "Reference the dll files in the bin folder", "None of the above"]
    ),
    Question(
        "What are the advantages of creating a project outside of the web root folder?",
        "All of the above",
        ["Separation of Concern: between solution files and Sitecore files", "Use the same Visual Studio project when you upgrade Sitecore or create a new instance.",
    ...
    Question(
        ...
        "Helix",
        ["Habitat", "Bootsrap", "Jquery", "Adaptive"]
    ),
    Question(
    ...
    Question(
        ...
        ["Use Implicit dependencies", "Use Sitecore Rocks", "Use Content Editor"]
    ),
    Question(
        "The layer concept in Helix provides a structure that is extremely suitable for",
        "All of the above",
        ["Creating and maintaining solutions of any size", "Produce maintenance-friendly and clean code",
         "Makes the dependency flow completely clear everywhere in the solution, in Sitecore, in Visual Studio and even in the file system."]
    ),
    Question(
    ...
    Question(
        "What layer of the Sitecore Architecture Conventions stitches the features of the solution together into a cohesive solution?",
        "Project",
        ["Feature", "Foundation", "Module"]
    ),
    Question(
    ...
    Question(
        ...
        ["Module-based Architecture", "Adaptive Architecture", "Helix-based Architecture"]
    ),
    Question(
        "Which service is responsible for processing and storing data?",
        "xDB",
        ["xConnect", "Reference Data Service", "xDB Processing"]
    ),
    Question(
        "Which of the following can you use to access the xDB core?",
        "xConnect Client API",
        ["Content Search API", "xDB API", "xDB Processing"]
    ),
    Question(
    ...
    Question(
        "What single endpoint name is used by the collection and Search in the development environment?",
        "xdb.collection",
        ["xdb.search", "xdb.collection search", "None of the above"]
    ),
    Question(
        "Where do you configure the xConnect endpoints?",
        "\\App_Config\\ConnectionStrings.config",
        ["\\App_Config\\Layers.config", "\\App_Config\\sitecore.config", "\\App_Config\\Web.config"]
    ),
    Question(
        "Which one of the following can help you connect to Sitecore using a Console application?",
        "xConnect Client API",
        ["xdb Service", "xdb collection service", "Content Search API"]
    ),
    Question(
        "Which model defines the CLR types, facets, and events?",
        "xConnect Collection model",
        ["xDB model", "Sitecore", "xConnect search"]
    ),
    Question(
    ...
    Question(
        ...
        ["Adding items", "Adding Templates"]
    ),
    Question(
        "Which one of the following can help you to control the types of items that users can insert beneath an existing item?",
        "Using the Insert Options",
        ["Design Layout", "Content Editor", "Experience Editor"]
    ),
    Question(
    ...
    Question(
        ...
        "Database",
        ["File System", "In an XML file", "None of the above"]
    ),
    Question(
    ...
    Question(
        "Items in Sitecore do not store website content.",
        "False",
        ["True"]
    ),
    Question(
    ...
    Question(
        "Which one the following do users need to specify when inserting an item from a template in Sitecore?",
        "Template Location, Item Name",
        ["Data Source", "Title Name"]
    ),
    Question(
        "Which user can create content anywhere in the tree?",
        "Administrators",
        ["Editors", "Marketers", "Users"]
    ),
    Question(
        "How can developers create content directly from Sitecore Explorer?",
        "Right-click an item > Add New Item",
        ["Right-click an item > Design Layout",
         "Right-click an item > Tasks > Add New Item",
         "Right-click an item > Tasks > Tools > Add New Item"]
    ),
    Question(
        ...
        ["Solution Explorer > Right-click an item > Add New Item > Click Add on the Add new dialog box",
         "Content Editor > Right-click an item > Add New Item > Click Add on the Add new dialog box",
         "None of the above"]
    ),
    Question(
    ...
    Question(
        ...
        ["User does not have proper rights", "User is not authenticated", "None of the above"]
    ),
    Question(
    ...
    Question(
        ...
        "Control Panel",
        ["Dashboard", "Marketing Panel", "Content Editor"]
    ),
    Question(
        "Each language version can have its own list of numbered versions.",
        "True",
        ["False"]
    ),
    Question(
        ...
        "Display Name",
        ["Context Menu", "Item Name", "None of the above"]
    ),
    Question(
        "How does Sitecore handle an item without a Display Name?",
        "Name is used as a fallback",
        ["Generate an exception", "Hides the item", "None of the above"]
    ),
    Question(
    ...
    Question(
        ...
        "Versioned",
        ["Shared", "Unversioned", "Single Versioned"]
    ),
    Question(
        "Which option will allow users to create a single version of the field value for all languages?",
        "Shared",
        ["Versioned", "Unversioned", "Single Versioned"]
    ),
    Question(
        "Which option will allow users to create a unique version of the field value per language?",
        "Unversioned",
        ["Versioned", "Shared", "Single Versioned"]
    ),
    Question(
    ...
    Question(
        "Sitecore automatically translates the labels, button text, etc. in other languages that you specify.",
        "False",
        ["True"]
    ),
    Question(
    ...
    Question(
        "Why is setting the icons and creating user-friendly names important when working with the templates in Sitecore?",
        "It's easier for the user to identify the template for creating items.",
        ["To impress users", "Save space on the server", "Templates will not render without icons"]
    ),
    Question(
        "What's the best way to define default values and settings for items?",
        "Use a base template",
        ["Use item fields", "Use Template's standard values", "None of the above"]
    ),
    Question(
        "What is the recommended practice for data Architecture?",
    ...
    Question(
        "What is the recommended practice for Content Structure?",
    ...
    Question(
        "What layer(s) does the Sitecore.Services.Client framework provide on both the Server and the Client side of the Sitecore applications for the developers to use to develop data-driven applications?",
        "Service",
        ["Project", "Foundation", "Data"]
    ),
    Question(
        "Which Web API does the Sitecore.Services.Client use as a foundation?",
        "ASP.NET Web API",
        ["Content Search API", "xConnect Client API", "xDB API"]
    ),
    Question(
        "What attribute needs to be set on the controller to enable the features of the Sitecore.Services.Client framework?",
        "[ServicesController]",
        ["[ClientController]", "[APIController]", "[WebController]"]
    ),
    Question(
        "What two services are provided by the Sitecore.Services.Client framework?",
        "ItemService, EntityService",
        ["DataTemplateService", "ConnectService"]
    ),
    Question(
        "What service of Sitecore.Services.Client can be used by the developer to gain access to regular Sitecore Items?",
        "ItemService",
        ["EntityService", "DataTemplateService", "ConnetService"]
    ),
    Question(
        "What service of Sitecore.Services.Client can be used to gain access to business objects that you define?",
        "EntityService",
        ["ItemService", "DataTemplateService", "ConnetService"]
    ),
    Question(
    ...
    Question(
        "What are the two types of items in the Sitecore database?",
        "Definition Items, Content Items",
        ["Items", "Web Items", "Templates"]
    ),
    Question(
        "Which item defines the configuration or structure of the implementation?",
        "Definition Item",
        ["Data Items", "Content Items", "None of the above"]
    ),
    Question(
        "In which environment are Definition items created and managed?",
        "Development",
        ["Production", "Test", "All of the above"]
    ),
    Question(
        "The Definition items are managed and created in the production environment and moved as part of versioned deployments from development to test to production.",
        "False",
        ["True"]
    ),
    Question(
        ...
        ["Layout and Rendering items", "Template and Field Items", "Placeholder and setting items", "Custom Field Types", "Lookup items for settings"]
    ),
    Question(
        "What items are managed by the editors on the website?",
        "Content Items",
        ["Definition Items", "Both a and b", "None of the above"]
    ),
    Question(
    ...
    Question(
        "Which content item is owned by production and should never be overwritten by an item coming from the development or test environment?",
        "Site home page item",
        ["Template", "Field", "None of the above"]
    ),
    Question(
    ...
    Question(
        ...
        "Serialization",
        ["Restore", "Versioning", "None of the above"]
    ),
    Question(
        "What tools are recommended to be used for item Serialization in Sitecore?",
        "TDS, Unicorn",
        ["Github", "Sitecore Rocks"]
    ),
    Question(
    ...
    Question(
        ...
        ["Sitecore Editor", "Sitecore Rocks"]
    ),
    Question(
        "Which tool can editors use to edit and write content directly on the page?",
        "Experience Editor",
        ["Content Editor", "Sitecore Rocks", "Sitecore Editor"]
    ),
    Question(
        "Which tool is best for more experienced content authors who are familiar with Sitecore and the functionality that it contains?",
        "Content Editor",
        ["Sitecore Rocks", "Experience Editor", "Sitecore Editor"]
    ),
    Question(
    ...
    Question(
        ...
        "Contains the content that all versions of the page, in all languages",
        ["Contains a combination of the specific for the current version of the page and the content that is specified in the shared layout.", "Both a and b", "None of the above"]
    ),
    Question(
        ...
        ["Contains the content that all versions of the page, in all languages", "Both a and b", "None of the above"]
    ),
    Question(
    ...
    Question(
        ...
        "Web database",
        ["Master database", "Core database", "xDB"]
    ),
    Question(
        "The item can only be published if all its ancestors have been published. Which option(s) should be checked when publishing an item in Sitecore?",
        "Publish subitems, Publish related items",
        ["Both a and b", "None of the above"]
    ),
    Question(
        "Which option will publish everything in Sitecore?",
        "Republish",
        ["Smart Publish", "Incremental Publish", "None of the above"]
    ),
    Question(
        "Which option will publish all items that have changed since the last publication?",
        "Smart Publish",
        ["Incremental Publish", "Republish", "None of the above"]
    ),
    Question(
        "Which option will publish only items that are in the publishing queue?",
        "Incremental Publish",
        ["Smart Publish", "Republish", "None of the above"]
    ),
    Question(
        "How many types of versions are there in Sitecore?",
        "Numbered Versions, Language Versions",
        ["Numeric Versions", "All of the above"]
    ),
    Question(
        "Which one of the following creates versions of an item in the same language?",
        "Numbered Versions",
        ["Language Versions", "Numeric Versions", "All of the above"]
    ),
    Question(
        "Which one of the following creates a version of an item in a different language?",
        "Language Versions",
        ["Numbered Versions", "Numeric Versions", "All of the above"]
    ),
    Question(
        ...
        "Clone",
        ["Copy", "Duplicate", "All of the above"]
    ),
    Question(
        "How do you identify the template that an item is based on?",
        "Content area > Content tab > Quick Info > Template",
        ["Content area > Advanced > Quick Info > Template", "Content area > Option > Quick Info > Template", "None of the above"]
    ),
    Question(
    ...
    Question(
        ...
        "Language Fallback",
        ["Multilanguage Fallback", "Both a and b", "None of the above"]
    ),
    Question(
        "The fallback language can be enabled on:",
        "Items, Fields",
        ["Data Types", "Home", "Content Tree"]
    ),
    Question(
    ...
    Question(
        ...
        "Item-level fallback",
        ["Field-level fallback", "Both a and b", "None of the above"]
    ),
    Question(
        "How can you improve the time it takes to load the Experience Editor ribbon?",
        "Change WebEdit.ShowNumberofLockedItemsOnButton value in Sitecore.ExperienceEditor.config to false",
        ["Toggle down the ribbon", "Change WebEdit.ShowNumberofLockedItemsOnButton value in Sitecore.config to false", "Change WebEdit.ShowNumberofLockedItemsOnButton value in Web.config to false"]
    ),
    Question(
        "What two frameworks does Sitecore support?",
        "MVC, Web Forms",
        ["JSON", "Bootstrap"]
    ),
    Question(
        ...
        "Nuget",
        ["Copy from the Sitecore installation", "Copy from Coworker's machine", "Contact Sitecore support"]
    ),
    Question(
    ...
    Question(
        "If the Sitecore page is displaying an error 'no layout', what could be the possible cause of this error?",
        "Presentation details are not set",
        ["Component .cshtml file is missing", "Template is not set on the item", "Template is not inherited from the base template"]
    ),
    Question(
        "Helix guidelines and conventions focus on architecture and mapping dependencies correctly.",
        "True",
        ["False"]
    ),
    Question(
        ...
        "Layout, Component",
        ["Layout", "Component", "None of the above"]
    ),
    Question(
        "Layout in Sitecore is:",
        "Scaffold of the page",
        ["Inner parts of the page", "Component", "Template"]
    ),
    Question(
        "Which property of an item stores the information that Sitecore requires to build a page response?",
        "Presentation Details",
        ["Item", "Sitecore framework", "Field"]
    ),
    Question(
        "Which ones form a layout in Sitecore?",
        ".cshtml file, Definition Item",
        ["Item", "Source Field", "None of the above"]
    ),
    Question(
        "Which field of the definition item links with the layout .cshtml file?",
        "Path field",
        ["Item field", "Model", "Source Field"]
    ),
    Question(
        "What Sitecore helper will you use to render field values to a page?",
        "Field",
        ["Item", "Template", "Data"]
    ),
    Question(
        "What rendering should only display logic?",
        "View Rendering",
        ["Controller Rendering", "Item Rendering", "Template Rendering"]
    ),
    Question(
        "Which one of the following is the default model used by the components in Sitecore?",
        "RenderingModel",
        ["ViewRendering", "FieldRendering", "ModelRendering"]
    ),
    Question(
        "The field helper will accept the following arguments:",
        "Field Name, Item, Set of parameters",
        ["Template Name, Field Name", "Content Items, Layout, parameter", "None of the above"]
    ),
    Question(
        ...
        "Context Page Item",
        ["Item", "Template Item", "None of the above"]
    ),
    Question(
        "What action will the field helper take if the data source item is not found?",
        "Fall back to the context item",
        ["Generate an exception", "Remove the item from the view", "None of the above"]
    ),
    Question(
        "You must use the RenderingModel in your view rendering to access a Sitecore item.",
        "False",
        ["True"]
    ),
    Question(
    ...
    Question(
        ...
        ["new {EnableWebEdit=false}", "new {WebEdit=false}", "new {DisableEdit=true}"]
    ),
    Question(
        "What parameter value can be set in the Sitecore Field Helper to set the width and height of an image?",
        "mw, mh",
        ["Width, height", "Width, length", "All of the above"]
    ),
    Question(
        "The source fields in Sitecore are used to:",
        "Limit the set of items that can be used",
        ["Pull data from the database", "Make it difficult for the user to change the values", "All of the above"]
    ),
    Question(
        "What templates define data types in Sitecore?",
        "Data Templates",
        ["Standard Template", "Base Template", "None of the above"]
    ),
    Question(
        "What template defines the base template for most data templates?",
        "Standard Template",
        ["Data Templates", "Base Template", "None of the above"]
    ),
    Question(
        "What field values are defined by the standard template?",
        "None",
        ["Standard values", "Default values", "Items"]
    ),
    Question(
        "The standard template inherits from a number of base templates defined under the /sitecore/templates/System/Templates/Sections item. Each of these base templates defines a single section of the standard template.",
        "True",
        ["False"]
    ),
    Question(
        "The names of fields in the standard template begin with:",
        "Two underscores",
        ["One underscore", "string", "Three underscores"]
    ),
    Question(
    ...
    Question(
        "How can developers allow content authors to edit complex fields such as Multilist, Checkbox, and Treelist fields using the Experience Editor?",
        "Field Editor",
        ["Item Editor", "Template Editor", "All of the above"]
    ),
    Question(
        "What is an Edit Frame?",
        "A region of the page that reacts to clicking by showing a toolbar",
        ["A component on that page to display item values", "A view of an image displayed within a page", "None of the above"]
    ),
    Question(
        "Which one of the following will help a developer add a button to any toolbar to display a field editor?",
        "Edit Frame button",
        ["Complex item", "Render item", "Edit Item button"]
    ),
    Question(
        "What is the most significant feature of the Sitecore system when designing the user experience in Sitecore?",
        "Dynamic layout engine",
        ["Static layout engine", "Data Templates", "All of the above"]
    ),
    Question(
        "How should a partial view be prefixed in Sitecore?",
        "Underscore",
        ["CamelCase", "User-friendly names", "All of the above"]
    ),
    Question(
        ...
        ["Create a Controller rendering definition item, Reference the Controller and Controller Action view by name", "All of the above"]
    ),
    Question(
        ...
        ["Create a View Rendering definition item, Point to the path of the .cshtml file", "All of the above"]
    ),
    Question(
    ...
    Question(
        ...
        "Both a and b",
        ["Static", "Dynamic", "None of the above"]
    ),
    Question(
        "When should you use static binding in Sitecore?",
        "Component always needs to be present on a layout or component",
        ["Do not have time to code", "Both a and b", "None of the above"]
    ),
    Question(
        "What binding should a developer use if a user wants to edit the component using the Experience editor?",
        "Dynamic",
        ["Static", "Adaptive", "All of the above"]
    ),
    Question(
        "Where do authors add, remove, move, and configure components?",
        "Experience Editor",
        ["Content Editor", "Both a and b", "None of the above"]
    ),
    Question(
        ...
        ["Cannot add any controls to the placeholders", "Will not be able to use placeholders", "None of the above"]
    ),
    Question(
        "The placeholder settings can:",
        "Make placeholders selectable in the Experience Editor, Are used to restrict components",
        ["Both a and b", "None of the above"]
    ),
    Question(
        "The items in the placeholder are called:",
        "Definition Items",
        ["Templates", "Items", "Information"]
    ),
    Question(
    ...
    Question(
        "What does Sitecore create when you override presentation details on the item?",
        "Layout Delta",
        ["Layout Details", "Presentation Details", "Template for the item"]
    ),
    Question(
        ...
        ["It creates different content in each placeholder", "It creates a unique key for each placeholder", "All of the above"]
    ),
    Question(
        ...
        ["Create static placeholders using @Html.Sitecore().Placeholder('Key')", "Change the Unique item in placeholder settings", "None of the above"]
    ),
    Question(
        "What can you do to create more flexible components on the page?",
        "Create dynamic placeholders",
        ["Create static placeholders", "Use view rendering", "All of the above"]
    ),
    Question(
        "Which standard field is a shared field where you specify the common layout for all languages and versions of the item?",
        "_Renderings",
        ["_Final Renderings", "_Item Renderings", "_Shared_Renderings"]
    ),
    Question(
        "Which standard field is a shared field where you specify individual layouts for languages and versions of items?",
        "_Final Renderings",
        ["_Renderings", "_Item Renderings", "_Shared_Renderings"]
    ),
    Question(
        "How are layouts stored in the fields?",
        "XML",
    ...
    Question(
        ...
        ["XML in _rendering field finalized", "When XML in the _Final Renderings finalized", "None of the above"]
    ),
    Question(
        "Which one of the following maps the item to the current URL?",
        "RenderingContext.Current.ContextItem",
        ["RenderingContext.Current.Rendering.item", "RenderingContext.Current.URL", "None of the above"]
    ),
    Question(
        "Which one of the following will help you to easily display contents from an item other than the context item?",
        "RenderingContext.Current.Rendering.item",
        ["RenderingContext.Current.ContextItem", "RenderingContext.Current.URL", "None of the above"]
    ),
    Question(
        "How can we prevent authors from selecting the wrong type of data source for the component?",
        "Set the Datasource Location",
        ["Set the Template Location", "Set Insert Options", "None of the above"]
    ),
    Question(
        "Where do you set up the Data Source location for a component?",
        "Component Toolbar > More > Edit Experience Editor Option",
        ["View > Advanced > Edit Experience Editor Option", "Home > Advanced > Edit Experience Editor Option", "Component Toolbar > More > Advanced > Edit Experience Editor Option"]
    ),
    Question(
    ...
    Question(
        "The Datasource Template cannot be configured with or without a Datasource location.",
        "False",
        ["True"]
    ),
    Question(
    ...
    Question(
        ...
        "Key-value pairs",
        ["XML", "HTML", "None of the above"]
    ),
    Question(
        "Which one of the following will allow developers to access the parameters of the component programmatically?",
        "@Model.Rendering.Parameters['CssClass']",
        ["@RenderingContext.Current.Rendering.Parameters['ButtonText']", "Both a and b", "None of the above"]
    ),
    Question(
        "How can developers help authors choose the correct component parameter values?",
        "Use fields",
        ["Key-value pair", "Both a and b", "None of the above"]
    ),
    Question(
        "What are the advantages of using Rendering Parameter templates?",
        "No free-form text input, Prevent errors, Control Input, Improve the user experience",
        ["All of the above"]
    ),
    Question(
        "How many parameter templates are allowed on a component?",
        "One",
        ["Two", "Three", "Unlimited"]
    ),
    Question(
    ...
    Question(
        ...
        ["Template fields", "Control Panel", "Configuration Items"]
    ),
    Question(
    ...
    Question(
        ...
        "Allows you to work with dynamic URLs, It matches any request, It ensures the URL is dynamic",
        ["All of the above"]
    ),
    Question(
    ...
    Question(
        "Developers should use ContentSearch for the best performance in Sitecore.",
        "True",
        ["False"]
    ),
    Question(
        "You should use 'var' in a foreach loop with the ChildList.",
        "False",
        ["True"]
    ),
    Question(
        "What should you use instead of 'var' in a foreach loop with the ChildList?",
        "Sitecore.Data.Items.Item",
        ["Sitecore.Data.Items.Context", "Site.Data.items.Item", "None of the above"]
    ),
    Question(
        "LINQ is useful in Sitecore for:",
        "Filtering small collections of items",
        ["Setting up configuration", "Reviews items", "None of the above"]
    ),
    Question(
    ...
    Question(
        ...
        ["Execute query right away", "Instruct Sitecore.Context not to execute the query", "None of the above"]
    ),
    Question(
        "What happens if you edit directly on the Web database?",
        "Changes are lost after a publish",
        ["The developers can no longer work", "Sitecore support will contact you", "Edited content automatically moves to Core database"]
    ),
    Question(
        "When you edit an item, how do you ensure that your changes are made on the Master database only?",
        "Point to the Master data using the following statement Var masterDB = Sitecore.Configuration.Factory.GetDatabase('master')",
        ["Change the database from the Sitecore Desktop view", "Change the database from the Sitecore Rocks plugin", "All of the above"]
    ),
    Question(
        "Anonymous visitors have permissions to create items in Sitecore.",
        "True",
        ["False"]
    ),
    Question(
        ...
        ["Sitecore.Context.Account.Switch('userid');", "None of the above"]
    ),
    Question(
    ...
    Question(
        "Which one of the following can you use when you need an endpoint to access and manipulate data remotely?",
        "Sitecore.Services.Client",
        ["Sitecore.Services.EndPoint", "Sitecore.Services.Context", "Sitecore.Services.DB"]
    ),
    Question(
        "Where can developers find JavaScript libraries?",
        "\\Sitecore\\shell\\client\\Services\\Assets\\lib",
        ["\\Sitecore\\shell\\configure\\Assets", "\\Sitecore\\shell\\Configure\\lib", "None of the above"]
    ),
    Question(
    ...
    Question(
        "The search interface operations allow you to:",
        "Remove search results, Run bulk operations over all the search results, Format search results",
        ["None of the above"]
    ),
    Question(
        "All fields are indexed by default in Sitecore.",
        "False",
        ["True"]
    ),
    Question(
        "What are the benefits of using Item buckets in Sitecore?",
    ...
    Question(
        ...
        ["/Home/System/Settings/Buckets/Item Buckets Settings", "/Content/System/Settings/Buckets/Item Buckets Settings", "/Dashboard/Control Panel/Settings/Buckets/Item Buckets Settings"]
    ),
    Question(
        "What API is used by the Search UI and Sitecore Buckets?",
        "ContentSearch API",
        ["ItemSearch Web API", "ContentSearch Web API", "Search Web API"]
    ),
    Question(
        "List two libraries that developers need to construct a search in Sitecore?",
        "Sitecore.ContentSearch, Sitecore.ContentSearch.Linq",
        ["Sitecore.Context.Item", "Both a and b", "None of the above"]
    ),
    Question(
    ...
    Question(
        "Which one of the following LINQ IQueryable methods are supported in Sitecore?",
        "Where, Contains, Any, All, OrderBy",
        ["GroupBy", "All of the above except a"]
    ),
    Question(
        "Why is it not recommended to use Axes.GetDescendants to retrieve a large number of items?",
        "Performance Issue",
        ["Less code", "Sitecore does not support", "Infrastructure cannot handle the query"]
    ),
    Question(
    ...
    Question(
        "Which one of the releases adds significant functionality to a product?",
        "Feature Release",
        ["Service Pack", "Update", "All of the above"]
    ),
    Question(
    ...
    Question(
        "How long will Sitecore support the Mainstream Support Phase?",
        "3 years",
        ["1 year", "2 years", "Forever"]
    ),
    Question(
        "Which tool will you use to analyze the log files?",
        "Log Analyzer",
        ["Event Viewer", "Sitecore Log Viewer", "Microsoft Word"]
    ),
    Question(
        "The self-test diagnostics can be run using:",
        "Sitecore Diagnostics Toolset",
        ["Sitecore Control Panel", "Windows System settings", "Sitecore Log Settings"]
    ),
    Question(
    ...
    Question(
        ...
        "Relevant Configuration Files, Recent Log files, Screenshots of UI, errors, and related files, Exceptions as plain text documents",
        ["All of the above"]
    ),
    Question(
        "What tool can be used to generate a support package for Sitecore support?",
        "Support Package Generator",
        ["Zip application", "Sitecore Tool Generator", "All of the above"]
    ),
